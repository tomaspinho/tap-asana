...
class Tasks(Stream):
    ...
    def get_objects(self):
        ...
        for workspace in self.call_api("workspaces"):
            # busco los ids de las tasks actualizadas recientemente
            for task in self.call_api("tasks.search_tasks_for_workspace", workspace_gid=workspace["gid"],
                                      params={"modified_at.after": modified_since}):
                task_data = self.call_api("tasks.get_task", task_gid=task["gid"])

                session_bookmark = self.get_updated_session_bookmark(session_bookmark, task_data[self.replication_key])
